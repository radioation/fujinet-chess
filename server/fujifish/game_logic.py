...
class GameState:
    ...
    def __init__( self, table: str, servername: str, use_bot: bool, register_lobby: bool ):
        ...
        if use_bot:
            ...
            # Todo, make this player configurable somehow
            level = 3 
    ...
    def add_player( self, player : str, is_bot:bool ) -> None:
        ...
        # if we have two players, activate player 
        if len(self.players) == self.max_players:
            self.active_player = 1 if self.use_bot == True else 0
    ...
    def do_move(self, uci_move:str, movetime_ms:float) -> None:
        try:
        ...
        if self.board.is_checkmate():
            self.message = "legal move: Check mate"
            return
        if self.board.is_stalemate():
            self.message = "legal move: Stale mate"
            return
            
        if self.board.is_insufficient_material():
            self.message = "legal move: Draw"
            return


        # get reply from stockfish.
        with chess.engine.SimpleEngine.popen_uci(os.getenv('ENGINE_PATH')) as eng:
            if self.use_bot:
                # configure skill level for single player
    ...
    def client_leave(self) -> None:
        if self.client_player < 0:
            return 
        self.message = f'{self.players[self.client_player].name} resigns'
